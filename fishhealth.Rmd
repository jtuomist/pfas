...
1. Amount should be gender and age-specific, because we need to worry about young mothers. Solution: take KKE amounts of fish, and scale those with the Goherr subgroup-specific proportions.
2. Amounts should reflect the actual fish consumption in Porvoo. Solution: postpone and use national statistics.
3. Infant's dioxin concentration module must be added. Solution: Use Goherr model for expo_indir
...
amount <- Ovariable(
  "amount",
  data = prepare(dat, "amount", c("Type","Response","Exposure_agent")),
  unit="Gg/a in Finland") # Filleted weight, i.e. no loss.
...
# Data from EU-kalat3: # pg/g fresh weight
...
conc_eukalat <- EvalOutput(Ovariable(
  ...
  data = data.frame(
    Alue = "Suomi",
    Havainto="PFOS",
    Result="2.056 (3.301 - 5.030)"),
  unit="ng/g fresh weight"
))

conc_pfas <- Ovariable(
  "conc_pfas",
  data=opbase.data("Op_fi5932",subset="PFAS-pitoisuudet"),
  unit="ng/g fresh weight")
conc_pfas@data$Alue <- "Porvoo"
conc_pfas <- EvalOutput(conc_pfas)

ggplot(conc_pfas@output, aes(x=conc_pfasResult, color=Havainto, linetype=Alue))+stat_ecdf()+
  scale_x_log10()+
...
sum_pfas <- oapply(conc_pfas, cols=c("Kala","Havainto"), FUN=sum)
tmp <- conc_pfas / sum_pfas
summary(tmp, marginals="Havainto")
...
exposure <- Ovariable(
  "exposure",
  dependencies = data.frame(Name=c("conc_vit", "amount", "fish_proportion")),
  formula = function(...) {
    ...
    amount <- amount * 1000 / 5.52 /365.25 # M kg/a per 5.52M population --> g/d per average person.
    
    out <- conc_vit * tmp * amount * fish_proportion
