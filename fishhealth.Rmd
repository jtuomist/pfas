...
25. Suomenna kaikki kuvat ja taulukot.
...
p <- ggplot(trim(tmp), aes(x=Group, weight=amountResult, fill=Fish))+geom_bar()+
  coord_flip()+ facet_grid(Area ~ .)+
  labs(
...
ggplot(tmp@output, aes(x=expo_dirResult+0.5, colour=Group))+stat_ecdf()+
  scale_x_log10() + facet_wrap(~Exposure_agent, scales="free_x", ncol=2)+
...
tmp <- trim(oapply(BoDattr,c("Area","Age","Iter"),sum))
tmp <- tmp[tmp$Area=="Finland",]
ggplot(tmp, aes(x=Age, y=-BoDattrResult, group=Area))+geom_line(colour="blue")+
  scale_x_discrete(breaks = levels(ages)[seq(1,length(ages),4)])+
  labs(
    title="Terveyshyöty kalasta ikäryhmittäin",
    x="Ikäryhmä",
    y="Terveystyöty (-DALY/a)"
  )
...
ggplot(tmp, aes(x0=x0, y0=y0, r=r, fill=Exposure_agent))+geom_circle(aes(color=Exposure_agent))+
  geom_text(aes(x=x0, y=y0, label=Response), hjust=-0.1)+
...
ggsave("Venn_raja_arvoylitykset_kala_altistumisessa.svg")
