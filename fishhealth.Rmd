...
BoDattr2 <- Ovariable(
  "BoDattr2",
  dependencies = data.frame(Name=c("BoDattr")),
  formula = BoDattrOrig)
BoDattr2 <- EvalOutput(BoDattr2, verbose = TRUE)

oprint(BoD@output[BoD$Iter=="500" & BoD$Gender=="Male" & BoD$Age=="70 - 74" , ])
oprint(BoDattr@output[BoDattr$Iter=="500" & BoDattr$Gender=="Male" & BoDattr$Age=="70 - 74" , ])
oprint(BoDattr2@output[BoDattr2$Iter=="500" & BoDattr2$Gender=="Male" & BoDattr2$Age=="70 - 74" , ])
...
ggplot(trim(BoDattr2), aes(x=Age, weight=BoDattr2Result, fill=Response))+geom_bar(position="stack")+
  scale_x_discrete(breaks = levels(ages)[seq(1,length(ages),3)])
...
ggplot(trim(oapply(BoDattr,c("Response","Exposure_agent","Iter"),sum)), 
       aes(x=Exposure_agent, y=BoDattrResult, color=Response, label=signif(BoDattrResult,2)))+geom_point()+
#  scale_x_discrete(breaks = levels(ages)[seq(1,length(ages),3)])+
